...
brand_list=['HDFC','ICICI','RBL','YES BANK']
...
rotation_threshold = 9
url_count = 0

proxy_list = [
    '18.132.190.56:8080',
    '117.54.114.33:80',
    # Add more proxy entries here
]
...
def rotate_proxy(driver):
    if proxy_list:
        ...
        proxy.http_proxy = proxy_list.pop(0)  # Get the next proxy from the list
        driver.proxy = proxy
    else:
        print("No more proxy")
credentials = [
...
def twitter_login(driver,username,password):
    ...
    time.sleep(3)
def update_url_count(ss_count, total_urls):
    url_window = Toplevel(root)
    url_window.title("URL Progress")
    url_label = Label(url_window, text=f"Current URL: {ss_count}/{len(total_urls)}")
    url_label.pack(padx=20, pady=10)
...
def linkedin_login(driver):
    ...
    WebDriverWait(driver,8).until(EC.visibility_of_element_located((By.NAME,'session_key')))
...
def talent_login(driver):
        driver.find_element(By.XPATH,'//*[@id="popupBackground"]/div/div/div/form/div[2]/div/button').click()
        time.sleep(2)
        value = driver.find_element(By.NAME,"phoneOrMailCheck")
        value.send_keys("rgarg4822@gmail.com")
        time.sleep(1)
        value.send_keys(Keys.ENTER)
        time.sleep(1)
        password=driver.find_element(By.NAME,"passwordLogin")
        password.send_keys("Pixel@2022")
        time.sleep(1)
...
def take_ss(source_urls,date,brand_name,category_name,missing_text,driver):
    ...
    for no,source in enumerate (source_urls,1):
        ...
        if re.search("https://www.linkedin.com",source) and linkedin_count ==1:
        ...
        if re.search("https://in.talent.com/",driver.current_url)and talent_count==1:
            time.sleep(10)
            talent_login(driver)
            talent_count+=1
        ...
        if category_name == "customer_care":
                ...
                try: 
                    ...
                    print("twitter_login  error ! ")
                    missing_source.append(source)
       
        elif "https://in.indeed.com" in source:
            driver.delete_all_cookies()
            # time.sleep(10) 
            s_ss = py.screenshot()
            s_ss.save('C:\HDFC_ICICI_Screenshot_Taker\source.jpg')
...
def crop_image(date,brand_name,category_name):
    global s_no
    img = Image.open(r"C:\HDFC_ICICI_Screenshot_Taker\source.jpg")
    img_size = img.size
...
def get_data():
    ...
    for i in range(0,len(source_urls)):
        update_url_count(ss_count,source_urls)
